...
int main(int argc, char *argv[])
{
    ...
    // CORNET virtual-port (TUN/TAP) contract — design D14/D15
    // The CORNET plugin passes --tun{i}=ifname,ip (one per middleware ip_list entry).
    // TAP bridge count equals arg count, NOT numUes.
    // Background UEs use NS-3 routing internally and get no TAP bridge.
    ...
    // CORNET virtual-port contract (design D14/D15): TAP bridge names and count
    // come from --tun{i}=name,ip CLI args injected by the CORNET plugin (TunManager).
    // Bridges are created only for CORNET-managed UEs (those in ip_list).
    // numUes may include background UEs that have no TAP bridge.
    ...
    {
        if (cornetTunArgs[i].empty()) break;   // no more --tun{i} args from plugin
