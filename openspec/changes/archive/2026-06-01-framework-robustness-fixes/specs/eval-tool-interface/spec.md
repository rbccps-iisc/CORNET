...
### Requirement: EvalTool provides a format_result() classmethod
The `EvalTool` base class SHALL provide a `format_result(value: float, status: str = "SUCCESS") -> str` classmethod that constructs a correctly formatted result string.

#### Scenario: format_result returns canonical string
- **WHEN** `EvalTool.format_result(14.3)` is called
- **THEN** it SHALL return `"SUCCESS, 14.3"`

#### Scenario: format_result with FAILURE status
- **WHEN** `EvalTool.format_result(0.0, status="FAILURE")` is called
- **THEN** it SHALL return `"FAILURE, 0.0"`

#### Scenario: format_result rejects non-float value
- **WHEN** `EvalTool.format_result("14.3 ms")` is called
- **THEN** it SHALL raise `TypeError` indicating that value must be a numeric float

## MODIFIED Requirements

### Requirement: EvalTool base class and per-task implementation
The framework SHALL provide `framework/eval/base.py` defining an abstract `EvalTool` class with a single required method `run_evaluation(results_dir: str) -> str`. The return value SHALL follow the format: first line `SUCCESS, <metric>` or `FAILURE,` optionally followed by `FAILURE, <metric>`, with additional detail lines permitted after the first. The `<metric>` SHALL be a numeric float string. The orchestrator's `collect()` phase SHALL raise `ValueError` if `<metric>` cannot be cast to `float`, with a message that includes the malformed string.
...
#### Scenario: Non-float metric string raises ValueError
- **WHEN** `run_evaluation()` returns `"SUCCESS, 14.3 ms"` or `"SUCCESS, high"` or any string whose metric portion cannot be cast to `float`
- **THEN** the orchestrator SHALL raise `ValueError` with a message identifying the malformed metric string
