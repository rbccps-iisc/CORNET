...
#### Scenario: Parallel sweep assigns isolated GAZEBO_MASTER_URI ports
- **WHEN** `experiment.sweep.parallel: true` and the config has a robot plugin
- **THEN** each variant SHALL receive `GAZEBO_MASTER_URI=http://localhost:{11345 + variant_index}` as an environment variable
- **THEN** no two parallel variants SHALL share a `GAZEBO_MASTER_URI` port
